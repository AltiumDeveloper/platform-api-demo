...
public partial class ConnectPage
{
    ...
    static async Task SetTokenAndWorkspaces(string token)
    {
        ...
        AltiumClientFactory.AccessToken = token;
        ...
        var client = AltiumClientFactory.GetClient(AppData.ApiEndpoint);
    ...
    async Task ConnectAsync()
    {
        ...
        try
        {
            ...
            try
            {
                await JS.InvokeVoidAsync("window.localStorage.setItem", GetTokenKey(), _token);
            }
            catch (Exception ex)
            {
    ...
    protected override async Task OnInitializedAsync()
    {
        ...
        // skip started
        _token = AltiumClientFactory.AccessToken;
        if (!string.IsNullOrEmpty(_token))
            return;
        ...
        try
        {
            ...
            else
            {
                ...
                try
                {
                    _token = await JS.InvokeAsync<string>("window.localStorage.getItem", GetTokenKey());
                }
                catch (Exception ex)
                {
                    Console.WriteLine(ex.Message);
    ...
    {
        return $"{AppData.Mode}|{AppData.ApiEndpoint}";
    }
}
